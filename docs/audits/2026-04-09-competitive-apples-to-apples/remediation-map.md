# Remediation Map

## Competitive score-surface hygiene

- `stale-workflow-teaching-comparator-identity`
  - [todo.md](../../../todo.md)
  - [docs/benchmark-comparison-scorecard.md](../../benchmark-comparison-scorecard.md)
  - [audit-summary.md](./audit-summary.md)

## Operator shell gap

- `operator-shell-gap-vs-runtime-shell-comparator`
  - [todo.md](../../../todo.md)
  - [audit-summary.md](./audit-summary.md)

## Workflow teaching gap

- `phase-teaching-gap-vs-workflow-teaching-comparator`
  - [todo.md](../../../todo.md)
  - [audit-summary.md](./audit-summary.md)

## Memory product gap

- `memory-product-gap-vs-memory-product-comparator`
  - [todo.md](../../../todo.md)
  - [docs/memoriesv2-long-term-architecture.md](../../memoriesv2-long-term-architecture.md)
  - [docs/open-source-memory-patterns.md](../../open-source-memory-patterns.md)

## Indexing gap

- `indexing-gap-vs-indexing-comparator`
  - [todo.md](../../../todo.md)
  - [docs/memoriesv2-long-term-architecture.md](../../memoriesv2-long-term-architecture.md)
  - [docs/benchmark-comparison-scorecard.md](../../benchmark-comparison-scorecard.md)
